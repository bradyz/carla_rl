...
def _preprocess_image(x):
    ...
    x = tv_utils.make_grid(x, padding=2, normalize=True)
    x = x.cpu().numpy()
...
class Experiment(object):
    ...
    def scalar(self, is_train=True, **kwargs):
        for k, v in sorted(kwargs.items()):
            ...
            if is_train:
                self._writer_train.add_scalar(k, v, self.epoch)
            else:
                self._writer_val.add_scalar(k, v, self.epoch)
    ...
    def end_epoch(self, net=None):
        for (is_train, k), v in self.scalars.items():
            ...
            if is_train:
                self._writer_train.add_scalar('%s_mean' % k, np.mean(v), self.epoch)
            else:
